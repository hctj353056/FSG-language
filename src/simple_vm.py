...
class VMState:
    ...
    memory: bytearray = field(
        default_factory=lambda: bytearray(MEMORY_SIZE)
    )
...
class Assembler:
    ...
    def __init__(self):
        ...
        self.pending_labels: Dict[str, List[Tuple[int, int]]] = (
            {}
        )  # 标签 -> [(地址, 位置)]
    ...
    def _second_pass(self, lines: List[str]):
        ...
        for line in lines:
            ...
            if line.startswith(".STR"):
                ...
                name = line.split()[1]
                self.symbols.append(
                    Symbol(name, 0, is_function=True, is_global=True)
                )
                continue
                        ...
                        if op == -1:
                            ...
                            for arg_str in arg_list:
                                if arg_str in self.labels:
                                    ...
                                    self.current_address += self._encode_imm(
                                        addr, self.output
                                    )
                                    imm_count += 1
                                    break
                                ...
                                # 标签未定义，使用当前地址
                                self.current_address += self._encode_imm(
                                    self.current_address, self.output
                                )
                                imm_count += 1
                        elif 0 <= op <= 7 and i == 0:
                            # 这是一个寄存器编码
    ...
    def _generate_bytecode(self) -> bytes:
        ...
        struct.pack_into(
            ">I", header, 12,
            len(self.output) - len(self.rodata)
        )  # text_size
        struct.pack_into(">I", header, 16, 0)  # data_size
        struct.pack_into(
            ">I", header, 20, len(self.rodata)
        )  # rodata_size
    ...
    def _parse_instruction(
        ...
        if mnemonic == "MOV":
            if len(args) == 2:
                return (OpCode.MOV,
                        [REG_MAP[args[0]], REG_MAP[args[1]]], 3)

        elif mnemonic == "LOADIMM":
            if len(args) == 2:
                return (
                    OpCode.LOADIMM,
                    [REG_MAP[args[0]], self._parse_value(args[1])],
                    5,
                )  # 5字节: opcode + reg + 4字节imm
...
class FSGVM:
    ...
    def run(self, entry: Optional[int] = None) -> int:
        ...
        while self.state.running and self.state.pc < len(
            self.state.memory
        ):
            try:
    ...
    def _load_bytecode_from_bytes(
        ...
        # 复制代码到内存
        text_offset = 0x20
        state.memory[0:text_size] = bytearray(
            bytecode[text_offset:text_offset + text_size]
        )

        # 复制 rodata
        rodata_offset = text_offset + text_size
        state.memory[0x1000:0x1000 + rodata_size] = bytearray(
            bytecode[
                rodata_offset:rodata_offset + rodata_size
            ]
        )
    ...
    def _read_imm32(self) -> int:
        ...
        val = struct.unpack(
            ">i",
            bytes(self.state.memory[self.state.pc:self.state.pc + 4]),
        )[0]
        self.state.pc += 4
        return val
    ...
    def _op_load(self):
        ...
        value = struct.unpack(
            ">i", bytes(self.state.memory[addr:addr + 4])
        )[0]
    ...
    def _op_push(self):
        ...
        self.state.sp -= 4
        struct.pack_into(
            ">i", self.state.memory, self.state.sp,
            self.state.registers[reg],
        )
        if self.debug:
    ...
    def _op_pop(self):
        ...
        reg = self._read_reg()
        value = struct.unpack(
            ">i", bytes(self.state.memory[self.state.sp:self.state.sp + 4])
        )[0]
        self.state.sp += 4
        self.state.registers[reg] = value
        if self.debug:
    ...
    def _op_mov(self):
        ...
        if self.debug:
            print(f"MOV R{dest}, R{src}"
                  f" = {self.state.registers[src]}")
    ...
    def _op_call(self):
        ...
        self.state.sp -= 4
        struct.pack_into(
            ">i", self.state.memory, self.state.sp, self.state.pc,
        )
        self.state.pc += offset - 1
        if self.debug:
    ...
    def _op_ret(self):
        ...
        addr = struct.unpack(
            ">i",
            bytes(self.state.memory[self.state.sp:self.state.sp + 4]),
        )[0]
        self.state.sp += 4
        self.state.pc = addr
        if self.debug:
    ...
    def _truncate_to_i32(self, value: int) -> int:
        """将值截断到 32 位有符号整数范围"""
        return (value & 0xFFFFFFFF) - (0x100000000 if value & 0x80000000 else 0)
        ...
        while True:
            try:
                ...
                if line.strip() == ".quit":
                    ...
                    print(f"Debug mode:"
                          f" {'ON' if self.vm.debug else 'OFF'}")
...
def disassemble(bytecode: bytes, count: int = 20) -> List[str]:
    ...
    while pc < len(bytecode) and len(instructions) < count:
        ...
        if opcode in mnemonics:
            instructions.append(
                f"0x{pc-1:04X}: {mnemonics[opcode]}"
            )
        else:
            instructions.append(
                f"0x{pc-1:04X}: DB 0x{opcode:02X}"
            )
...
def main():
    ...
    if filepath.startswith("fsg{") and filepath.endswith("}"):
        ...
        for try_path in [
            filepath,
            os.path.join("FSG语言", filepath),
            os.path.join("FSG语言", "examples", filepath),
        ]:
            if os.path.exists(try_path):
                filepath = try_path
                break
    ...
    if os.path.exists(filepath):
        ...
        if header[:4] == b"FSGB":
            ...
            source = preprocess_source(
                source, os.path.dirname(os.path.abspath(filepath))
            )
            ...
            print(f"✓ 编译成功: {filepath} -> {output_path}"
