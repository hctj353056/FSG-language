...
class Token:
    ...
    def __repr__(self):
        return f"Token({self.type.name}, '{self.value}', L{self.line}:C{self.column}, [{self.layer}])"
...
class FSGTokenizer:
        ...
        while pos < len(code):
            ...
            for pattern, token_type in self.TOKEN_PATTERNS:
                ...
                if match:
                    ...
                    self.tokens.append(Token(
                        token_type, value, line, start_col, layer
                    ))
    ...
    def _detect_layer(self, code: str) -> None:
        ...
        # 高级层特征
        high_chars = set('声明模块引定义流程输入输出获取使用传入得到依次若则否则当循环返回且或非真的假的空中文')
        high_patterns = ['"', '"', '（', '）', '：', '；', '，']
...
def print_token_table(tokens: List[Token]) -> None:
    ...
    for token in tokens:
        ...
        print(f"│ {layer:<6} │ {type_name:<8} │ {value} │ {token.line:>4} │ {token.column:>4} │")
...
使用.求平均数
        ...
        print(f"Token 总数: {len([t for t in tokens if t.type != TokenType.WHITESPACE and t.type != TokenType.EOF])}")
        ...
        if tokenizer.errors:
            ...
            for err in tokenizer.errors:
                print(f"  位置 {err['line']}:{err['column']} - 未知字符 '{err['char']}'")
