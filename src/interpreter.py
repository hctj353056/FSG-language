    ...
    def read_string(self, quote):
        ...
        while self.pos < len(self.code):
            ...
            # 遇到结束引号，减少嵌套计数
                ...
                raise LexerError("字符串未闭合", start_line, start_column)
            else:
                result.append(self.advance())
        raise LexerError("字符串未闭合", start_line, start_column)
    ...
    def tokenize(self):
        ...
        while self.pos < len(self.code):
            ...
            if ch in "\"'" or ch == "\u201c" or ch == "\u201d":
                tokens.append(
                    Token(
                        TokenType.STRING, self.read_string(ch), start_line, start_column
                    )
                )
            # 数字（可能带扩展名如001.fsg）
            elif ch.isdigit():
                ...
                if num_str.isdigit():
                    tokens.append(
                        Token(TokenType.NUMBER, int(num_str), start_line, start_column)
                    )
                elif "." in num_str:
                    parts = num_str.split(".")
                    if parts[0].isdigit() and parts[1].isdigit():
                        ...
                        tokens.append(
                            Token(
                                TokenType.IDENTIFIER, num_str, start_line, start_column
                            )
                        )
                else:
                    tokens.append(
                        Token(TokenType.IDENTIFIER, num_str, start_line, start_column)
                    )
            # 标识符/关键字（支持中文）
            elif (
                ch.isalpha() or ch == "_" or ("\u4e00" <= ch <= "\u9fff" and ch != "：")
            ):
            ...
            # 符号
                ...
                if ch == "：":
                    tokens.append(
                        Token(TokenType.SYMBOL, "COLON", start_line, start_column)
                    )
                elif ch == "；":
                    tokens.append(
                        Token(TokenType.SYMBOL, "SEMICOLON", start_line, start_column)
                    )
                elif ch == "，":
                    tokens.append(
                        Token(TokenType.SYMBOL, "COMMA", start_line, start_column)
                    )
                else:
                    tokens.append(
                        Token(
                            TokenType.SYMBOL, self.SYMBOLS[ch], start_line, start_column
                        )
                    )
            else:
