...
class IRValue:
    ...
    def to_dict(self):
        return {"name": self.name, "type": str(self.type_info) if self.type_info else None}
...
class IRPhi(IRInstruction):
    """Phi 节点（SSA 分支合并）"""
    branches: List[Tuple[str, IRValue]] = field(default_factory=list)  # [(label, value), ...]
...
class IRFunction:
    ...
    params: List[Tuple[str, IRTypeInfo]] = field(default_factory=list)  # [(name, type), ...]
    ...
    def parse(self, source: str) -> IRModule:
        ...
        while i < len(lines):
            ...
            # 输入语句 → PRINT + INPUT
                ...
                current_block.add_inst(IRConst(
            ...
            # 获取...个数 → len
                ...
                if match:
                    ...
                    arr_ref = ssa.get_value(var_name) if var_name in ssa.versions else IRValue(var_name)
                    current_block.add_inst(IRConst(
                        opcode=IROpcode.LEN,
                        result=len_var,
                        operands=[arr_ref]
            ...
            # 输出语句 → PRINT
                ...
                if match:
                    ...
                    var = ssa.get_value(var_name) if var_name in ssa.versions else IRValue(var_name)
                    current_block.add_inst(IRConst(
...
class IRParser:
    ...
    TOKEN_PATTERNS = [
        ...
        ('BINARY', r'%(\w+)\s*=\s*(add|sub|mul|sdiv|udiv|srem|urem|and|or|xor|shl|lshr|ashr)\s+(%?\w+),\s*(%?\w+)'),
...
"""
    ...
    entry.add_inst(IRConst(opcode=IROpcode.CONST, result=i, operands=[IRValue("0")], const_value=0))
    entry.add_inst(IRConst(opcode=IROpcode.CONST, result=sum_val, operands=[IRValue("0")], const_value=0))
    entry.add_inst(IRBr(opcode=IROpcode.BR, target="loop"))
    
    # loop: 判断和累加
    ssa_i = ssa.new_value("i", IRTypeInfo(IRType.INT))
    ssa_sum = ssa.new_value("sum", IRTypeInfo(IRType.INT))
    cmp_result = ssa.new_value("cmp", IRTypeInfo(IRType.BOOL))
    
    loop.add_inst(IRConst(opcode=IROpcode.ICMP_SLE, result=cmp_result, operands=[ssa_i, IRValue("10")]))
    loop.add_inst(IRBr(opcode=IROpcode.COND_BR, cond=cmp_result, true_target="exit", false_target="loop"))
...
class PythonLower(LanguageLower):
    ...
    def _lower_function(self, node: ast.FunctionDef) -> IRFunction:
        ...
        for stmt in node.body:
            insts, current_block = self._lower_statement(
                stmt, current_block, ssa
            )
            for inst in insts:
                current_block.add_inst(inst)
    ...
    def _lower_statement(self, node: ast.stmt, 
        ...
        if isinstance(node, ast.Assign):
            ...
            if isinstance(node.op, ast.Add):
                insts.append(IRConst(opcode=IROpcode.ADD, result=result, 
                                     operands=[target_val, right_val]))
            elif isinstance(node.op, ast.Sub):
                insts.append(IRConst(opcode=IROpcode.SUB, result=result,
                                     operands=[target_val, right_val]))
            elif isinstance(node.op, ast.Mult):
                insts.append(IRConst(opcode=IROpcode.MUL, result=result,
                                     operands=[target_val, right_val]))
            elif isinstance(node.op, ast.Div):
                insts.append(IRConst(opcode=IROpcode.DIV, result=result,
                                     operands=[target_val, right_val]))
    ...
    def _lower_expr(self, node: ast.expr,
        ...
        if isinstance(node, ast.Constant):
            ...
            if isinstance(node.op, ast.Add):
                insts.append(IRConst(
                    opcode=IROpcode.ADD, result=result,
                    operands=[left_val, right_val]
                ))
            elif isinstance(node.op, ast.Sub):
                insts.append(IRConst(
                    opcode=IROpcode.SUB, result=result,
                    operands=[left_val, right_val]
                ))
            elif isinstance(node.op, ast.Mult):
                insts.append(IRConst(
                    opcode=IROpcode.MUL, result=result,
                    operands=[left_val, right_val]
                ))
            elif isinstance(node.op, ast.Div):
                insts.append(IRConst(
                    opcode=IROpcode.DIV, result=result,
                    operands=[left_val, right_val]
                ))
            elif isinstance(node.op, ast.Mod):
                insts.append(IRConst(
                    opcode=IROpcode.REM, result=result,
                    operands=[left_val, right_val]
                ))
...
class JSLower(LanguageLower):
    ...
    def _lower_js_function(self, name: str, params: List[str], body: str) -> IRFunction:
        ...
        for var_match in re.finditer(var_pattern, body):
            ...
            value_insts, value, current_block = self._lower_js_expr(expr, current_block, ssa)
    ...
    def _lower_js_expr(self, expr: str, block: IRBlock, 
                       ssa: SSAValueManager
                       ) -> Tuple[List[IRInstruction], IRValue, IRBlock]:
        """降级 JS 表达式"""
        insts = []
        ...
        # 算术运算
            ...
            if match:
                left_insts, left_val, block = self._lower_js_expr(match.group(1), block, ssa)
                right_insts, right_val, block = self._lower_js_expr(match.group(2), block, ssa)
                insts.extend(left_insts)
                insts.extend(right_insts)
...
class GoLower(LanguageLower):
    ...
    def _lower_go_function(self, name: str, 
        ...
        for var_match in re.finditer(var_pattern, body):
            ...
            value_insts, value, current_block = self._lower_go_expr(expr, current_block, ssa)
            for inst in value_insts:
                current_block.add_inst(inst)
    ...
    def _lower_go_expr(self, expr: str, block: IRBlock,
                      ssa: SSAValueManager
                      ) -> Tuple[List[IRInstruction], IRValue, IRBlock]:
        """降级 Go 表达式"""
        insts = []
        ...
        if re.match(r'^-?\d+\.?\d*$', expr):
            is_float = '.' in expr
            val = ssa.new_value("const", 
                               IRTypeInfo(IRType.FLOAT if is_float else IRType.INT))
            insts.append(IRConst(
                opcode=IROpcode.CONST,
                result=val,
                operands=[IRValue(expr)],
        ...
        # 算术运算
            ...
            match = re.match(r'^(.+)\s*\+\s*(.+)$', expr)
            if match:
                left_insts, left_val, block = self._lower_go_expr(match.group(1), block, ssa)
                right_insts, right_val, block = self._lower_go_expr(match.group(2), block, ssa)
                insts.extend(left_insts)
                insts.extend(right_insts)
                result = ssa.new_value("add", IRTypeInfo(IRType.INT))
                insts.append(IRConst(
                ...
                while i < len(block.instructions):
                    ...
                    if isinstance(inst, IRRet):
                        ...
                        while j >= 0:
                            ...
                            if isinstance(prev, IRConst) and prev.opcode == IROpcode.STORE:
                                if prev.operands and str(prev.operands[0]) not in used_values:
                                    pass
                            j -= 1
                    i += 1
